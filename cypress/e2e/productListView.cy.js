...
describe("Check product list view", function () {
 ...
 it('shoudl display products sorted by ascending price', () => {
     ...
     cy.xpath('//div[contains(@class, "list row")]/div/div/div/div[2]/div[4]/div[@class="price"]').then(($prices) => {
    ...
    it('should displayed product details in each card', () => {
        ...
        cy.xpath('//div[contains(@class, "list row")]/div/div/div/div[2]/a[@class="prdocutname"]').each((card)=> {
      ...
      it('should displayed product images in each card', () => {
        // Get all the product cards on the page
        cy.xpath('//div[contains(@class, "list row")]/div/div/div/div/a/img').each((card)=> {
            // Assert that the product name is visible within the card
            cy.wrap(card).should('be.visible')
        })
      })
      ...
      it('should displayed product prices in each card', () => {
        // Get all the product cards on the page
        cy.xpath('//div[contains(@class, "list row")]/div/div/div/div[2]/div[4]/div[@class="price"]').each((card)=> {
            // Assert that the product name is visible within the card
            cy.wrap(card).should('be.visible')
        })
      })
